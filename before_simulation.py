# Every function called before simulation is defined here.
...
def create_response_layer(neuron_count, neuron_eqs, pool_capacity):
    response_layer = NeuronGroup(1, neuron_eqs, threshold='v>v_th', reset='v = beta*v_th', method='euler')
...
def create_synapse_objects(syn_eqs, hidden_layers, input_layer, on_pre_arg, initial_weights):
    ...
    arr = [Synapses(input_layer, hidden_layers[0], model=syn_eqs, on_pre=on_pre_arg)]
    arr[0].connect()
    ...
    for i in range(1, hidden_layers_len - 1):
        arr.append(Synapses(hidden_layers[i], hidden_layers[i + 1], model=syn_eqs, on_pre=on_pre_arg))
        arr[i].connect()
